	...
	while (fCurrentDamage > 0)
	{
		...
		if (player == target_player && tr.hitgroup != HITGROUP_GENERIC && tr.hitgroup != HITGROUP_GEAR)
		{
			return scale_damage(fCurrentDamage, tr.hitgroup, player->get_armor_value(),
				player->get_has_helmet(), weapon_data->m_flArmorRatio) > Options.Ragebot.Aimbot.aimbot_mindmg;
		}
...
void caimbot::noSpread (CUserCmd* pCmd, bool& sendpacket)
{
	if (!Options.Misc.Run)
	return;
	auto local_player = reinterpret_cast<C_CSPlayer*>(g_entitylist->GetClientEntity(g_engine->GetLocalPlayer()));
	if (!local_player || local_player->IsDormant() || local_player->get_team_num() == 1)
		return;
...
void caimbot::move(CUserCmd* pCmd, bool& sendpacket)
{
	...
	if (Options.Ragebot.Aimbot.pSilentAim)
	{
		const auto server_time = local_player->get_tick_base() * g_globals->interval_per_tick;
		if (server_time <= weapon->get_next_primary_attack() || server_time <= local_player->get_next_attack())
			return;
	...
	auto hitboxpos = entity::get_hitbox_position(entity, Options.Ragebot.Aimbot.hitbox);
	...
	if (Options.Ragebot.Aimbot.aimbot_smooth > 1)
	{
		delta = angles - current_angles;
		math::clamp_angles(delta);
		delta /= Options.Ragebot.Aimbot.aimbot_smooth;
		math::clamp_angles(delta);
	...
	if (!Options.Ragebot.Aimbot.aimbot_on_key || (Options.Ragebot.Aimbot.aimbot_on_key && GetAsyncKeyState(Options.Ragebot.Aimbot.key)))
	{
		if (Options.Ragebot.Aimbot.aimbot_on_key && !GetAsyncKeyState(Options.Ragebot.Aimbot.key))
			Options.Misc.Run = true;
		else 
			Options.Misc.Run = false;
		...
		pCmd->viewangles = angles;
		if (!Options.Ragebot.Aimbot.bSilentAim)
			g_engine->SetViewAngles(angles);
		else if (Options.Ragebot.Aimbot.pSilentAim)
			sendpacket = false;

		if (Options.Ragebot.Aimbot.bAutoShoot)
			if (!weapon_data->bFullAuto)
			{
				if (pCmd->tick_count % 2)	pCmd->buttons |= IN_ATTACK;
...
C_CSPlayer* caimbot::get_best_target()
{
	...
	auto bestfov = Options.Ragebot.Aimbot.Fov;
	...
	for (auto i = 0; i < g_entitylist->GetHighestEntityIndex(); i++)
	{
		...
		if (clientclass->m_ClassID != CCSPlayer)
			continue;
		if (Options.Ragebot.Aimbot.iHitbox == 0)
			Options.Ragebot.Aimbot.hitbox = 12;
		else if (Options.Ragebot.Aimbot.iHitbox == 1)
			Options.Ragebot.Aimbot.hitbox = 11;
		else if (Options.Ragebot.Aimbot.iHitbox == 2)
			Options.Ragebot.Aimbot.hitbox = 10;
		else if (Options.Ragebot.Aimbot.iHitbox == 3)
			Options.Ragebot.Aimbot.hitbox = 9;

		auto delta = local->get_eye_position() - entity::get_hitbox_position(entity, Options.Ragebot.Aimbot.hitbox);
		Vector angles;
		math::VectorAngles(delta, angles);
		math::clamp_angles(angles);
		...
		auto dist = local->get_origin().DistTo(entity->get_origin());
		if (Options.Ragebot.Aimbot.iSelection == 0)
		{
			if (dist < bestdist)
			{
				bestdist = dist;
